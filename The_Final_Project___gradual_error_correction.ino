...
int errorTime; // how much time the robot has been in error.
int errorBegin; // the millis() value at the beginning of the current error

int correctionSpeed; //values -255 up to 255, used to control the correction speed adjusted for error time.
...
void errorCheck() {  //this also includes functionality to update the variable "errorTime" 
  read_sensor1();
  read_sensor2();
...
boolean errorCorrection() { // doesn't actually return anything meaningful rhe return is just used to break out of the function
// this version include changes to make this function correct gradually.
  errorCheck();
  
  correctionSpeed = 0.1275 * errorTime; //if adjustions need to be made they should be made here.
 
  //adjust to analogWrite parameters.
  if(correctionSpeed > 255)
    correctionSpeed = 255;
  if(correctionSpeed < -255)
    correctionSpeed = -255; 
    ...
    while(ER && EL) { // while both sensors are off track 
      errorCheck();
      move('l', correctionSpeed);
      move('r', correctionSpeed);
      if(!EL || !ER) 
      return true;
    }
